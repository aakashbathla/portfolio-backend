...
export default function User() {
  ...
  const getUserPosts = async () => {
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BASE_URL}api/users/posts/${currentUser.id}`
      );
      setPosts(res.data);
    } catch (error) {
      console.log(error);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.put(
        `${process.env.REACT_APP_BASE_URL}api/users/deactivate-post/${id}`,
        { isActive: 0 },
        { withCredentials: true }
      );
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.put(
        ...
        { user_name: userName, email: email, user_img: img, id: state.id },
        { withCredentials: true }
      );
      if (res.status === 200) {
          ...
          posts.map((post) => (
            <div key={post.id}>
              <PostCard
                ...
                linkTo={`/write?edit-post=${post.id}`}
              />
            </div>
          ))}
      </div>
